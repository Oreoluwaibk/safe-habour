...
const WeeklyTemplate = () => {
  ...
  const handleSilentGetAvailabilty = useCallback(() => {
    getSchedule()
    .then(res => {
      if(res.status === 200) {
        setLoading(false);
  ...
  return (
    ...
    >
      ...
      <Row gutter={[15, 15]}>
        {dayOfWeek.map((days: {id: number; title: string; name: string}, i: number) => (
          <Col lg={24} sm={24} xs={24} key={i}>
            <WeeklyCard refresh={handleSilentGetAvailabilty} allSchedule={allSchedule} available={true} title={days.name}  />
          </Col>
        ))}
      </Row>
